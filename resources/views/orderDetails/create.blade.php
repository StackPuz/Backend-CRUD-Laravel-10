            ...
            <form method="post" action="/orderDetails?ref={{urlencode($ref)}}">
                ...
                <div class="row">
                    <div class="form-group col-md-6 col-lg-4">
                        ...
                        <input id="order_detail_order_id" name="order_id" class="form-control form-control-sm" value="{{old('order_id')}}" type="number" required />
                        @error('order_id')<span class="text-danger">{{$message}}</span>@enderror
                    </div>
                    <div class="form-group col-md-6 col-lg-4">
                        <label for="order_detail_no">No</label>
                        <input id="order_detail_no" name="no" class="form-control form-control-sm" value="{{old('no')}}" type="number" required />
                        @error('no')<span class="text-danger">{{$message}}</span>@enderror
                    </div>
                    <div class="form-group col-md-6 col-lg-4">
                        <label for="order_detail_product_id">Product</label>
                        <select id="order_detail_product_id" name="product_id" class="form-control form-control-sm" required>
